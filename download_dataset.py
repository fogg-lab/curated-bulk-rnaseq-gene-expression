...
URLS_LIST_FILE = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "dataset_urls.txt"
)
            ...
            try:
                ...
                if success:
                    ...
                    if files_downloaded % PROGRESS_INTERVAL == 0:
                        ...
                        print(
                            f"Progress: {progress_percent}% ({files_downloaded}/{len(urls)} files downloaded)"
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download a dataset folder from AWS S3."
    )
